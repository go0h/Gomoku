...
Gomoku::Gomoku() : _mode(PvE),
                   _player(WHITE),
                   _difficult(EASY),
                   _board(Board(19)),
                   _minimax(Minimax(NUM_THREADS)) {}

Gomoku::Gomoku(t_gomoku_mode mode, t_color color, t_difficult difficult, size_t board_size) : _mode(mode),
                                                                                              _player(color),
...
GomokuMethod Gomoku::_print_hints(MethodArgs::pointer args)
{
  ...
  // std::cout << "_difficult=" << _difficult <<  "_player = " << _player << std::endl;
  for (size_t i = 0; i < MAX_MOVES_HINTS && i < moves.size(); ++i)
  {
    hint->hints.push_back(_board.coord_to_pos(moves[i].x, moves[i].y));
    // std::cout << "turn = " << _board.coord_to_pos(moves[i].x, moves[i].y) << ", score = " << moves[i].score << std::endl;
  }
...
MakeTurn *Gomoku::_create_turn(std::vector<t_move_eval> &moves)
{
  ...
  //  std::cout << "_difficult=" << _difficult <<  "_player = " << _player << std::endl;
  //   std::cout << "turn = " << _board.coord_to_pos(best_move.x, best_move.y) << ", score = " << best_move.score << std::endl;
...
std::vector<std::string> Gomoku::_set_move_and_get_captures(Board &state, long x, long y, t_color player)
{

  long side = static_cast<long>(state.getSide());
  ...
  for (long i = 0; i != 8; ++i)
  {
    ...
    if (x3 > -1 && x3 < side && y3 > -1 && y3 < side && state(x3, y3) == player)
    {
...
void Gomoku::_print_config()
{
  ...
  std::cout << GREEN << "Opponent captures:\t" << CYAN << _captures[(_player == WHITE ? BLACK : WHITE)] << RESET << std::endl;
}
