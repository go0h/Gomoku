#include <cstring>

#include "Board.hpp"
#include "GomokuException.hpp"

/*
** CONSTRUCTORS AND DESTRUCTORS
*/

Board::Board(t_point side) :
  _field(new t_point[side * side]),
  _score(0),
  _side(side) { }

Board::Board(Board const & src)
  : _field(new t_point[src._side * src._side]),
  _score(src._score),
  _side(src._side) { }

Board::Board(Board && src) noexcept
{
  *this = std::move(src);
}

Board::~Board()
{
  delete [] _field;
}

/*
** OPERATORS
*/

Board&  Board::operator=(Board const & src)
{
  if (this == &src)
    return *this;
  Board temp(src);
  temp.swap(*this);
  return *this;
}

Board&  Board::operator=(Board && src) noexcept
{
  if (this == &src)
    return *this;
  this->swap(src);
  return (*this);
}

bool  Board::operator==(Board const & n) const
{
  return !std::memcmp(_field, n._field, sizeof(t_point) * _side * _side);
}

bool  Board::operator!=(Board const & n) const
{
  return !(*this == n);
}

bool  Board::operator<(Board const & n) const
{
  if (_score < n._score)
    return true;
  if (!(_score - n._score))
    return true;
  return false;
}

t_point&   Board::operator()(size_t i, size_t j)
{
  if (i >= _side && j >= _side)
    throw GomokuException(std::string(E_INDEX));
  return _field[i * _side + j];
}

t_point    Board::operator()(size_t i, size_t j) const
{
  if (i >= _side && j >= _side)
    throw GomokuException(std::string(E_INDEX));
  return _field[i * _side + j];
}

Board::operator bool()
{
  return _field;
}

/*
** SETTERS AND GETTERS
*/

void    Board::setScore(size_t score) { this->_score = score; }

/*
** UTILS METHODS
*/

void    Board::printBoard() const
{
  for (size_t i = 0; i != _side; ++i) {
    for (size_t j = 0; j != _side; ++j) {
      char p = '.';
      if ((*this)(i, j) == WHITE)
        p = 'W';
      else if ((*this)(i, j) == BLACK)
        p = 'B';
      printf("%-2c", p);
    }
    printf("\n");
  }
  printf("\n");
}

void    Board::swap(Board & src)
{
  std::swap(_score, src._score);
  std::swap(_field, src._field);
  std::swap(_side, src._side);
}

size_t    Board::getHash() const
{
  size_t hash = 0;
  size_t pow = 31;
  for (size_t i = 0; i != (_side * _side); ++i)
  {
    hash = hash + (_field[i] * pow);
    pow *= 31;
  }
  return hash;
}
